...
def run_mcmc(fn, t0_lim=None):
    ...
    def create_logL(cube):
        ...
        logL = np.sum(np.log(1. / np.sqrt(2. * np.pi * sigma_sq)) - 0.5 * (f_model - fdata)**2 / sigma_sq)
        return logL
...
def run_curve_fit(fn):
    ...
    bounds = ([max_flux * 10**(-0.2), 0., -2., np.amin(tdata) - 50., -1.0, 0.5], \
              [max_flux * 10**(1.2), 0.02, 2.5, np.amax(tdata) + 50., 3., 4.])
    p0 = np.array([max_flux, 0.0052, 1.1391, max_flux_loc, 0.5990, 1.4296, 1.0607, 1.0424, np.log10(1.0075), 1.0006, np.log10(0.9663), np.log10(0.5488)])
    ...
    def flux_model_smooth(t_data, A, beta, gamma, t0, tau_rise, tau_fall):
        ...
        phase = t_data - t0    
        f_model = A / (1. + np.exp(-phase / tau_rise)) * (1. - beta * gamma) * np.exp((gamma - phase) / tau_fall) * sigma
        f_model += A / (1. + np.exp(-phase / tau_rise)) * (1. - beta * phase) * (1. - sigma)

        return f_model

    popt_r, pcov = curve_fit(flux_model_smooth, tdata[bdata == "r"], fdata[bdata == "r"], p0=p0[:6], sigma=ferrdata[bdata == "r"], bounds=bounds, maxfev=100000)
    ...
    popt_g, pcov = curve_fit(flux_model_smooth, tdata[bdata == "g"], fdata[bdata == "g"], p0=p0_g, sigma=ferrdata[bdata == "g"], bounds=bounds, maxfev=100000)
...
def main_loop_single_file(test_fn, output_dir):
    try:
