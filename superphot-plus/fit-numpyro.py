...
def run_mcmc(fn, sampler="NUTS", t0_lim=None, plot=False):
    ...
    def jax_guide(t = None, obsflux = None, uncertainties = None, max_flux = None, inc_band_ix = None):
    ...
    if sampler == "NUTS":
        ...
        ns.run(random.PRNGKey(1), obsflux = fdata, t=tdata, uncertainties=ferrdata, max_flux=max_flux, inc_band_ix=inc_band_ix)
        ...
        num_iter = 10000
        with numpyro.validation_enabled():
            svi_result = svi.run(random.PRNGKey(1), num_iter, stable_update=True, obsflux = fdata, t=tdata, uncertainties=ferrdata, max_flux=max_flux, inc_band_ix=inc_band_ix)
        params = svi_result.params
        posterior_samples = {}
        for p in params:
            if p[-2:] == "mu":
                posterior_samples[p[:-3]] = np.random.normal(loc=params[p], scale=params[p[:-2] + "sigma"], size=100)
    ...
    discrete_samples = predictive(random.PRNGKey(1), t=tdata_stacked, uncertainties=ferrdata_stacked, max_flux=max_flux, inc_band_ix=inc_band_ix)
    ...
    if plot:
        ...
        model_i = np.array([{k: posterior_samples[k][j] for k in posterior_samples.keys()} for j in range(len(posterior_samples['log_tau_fall']))])

        plt.errorbar(tdata[bdata==0], fdata[bdata==0], yerr=ferrdata[bdata==0], c="g", label="g", fmt="o")
        plt.errorbar(tdata[bdata==1], fdata[bdata==1], yerr=ferrdata[bdata==1], c="r", label="r", fmt="o")

        trange_fine = np.linspace(np.amin(tdata), np.amax(tdata), num=500)

        for sample in model_i[:30]:
            plt.plot(trange_fine, flux_from_posteriors(trange_fine, sample, max_flux)[0], c="g", lw=1, alpha=0.1)
            plt.plot(trange_fine, flux_from_posteriors(trange_fine, sample, max_flux)[1], c="r", lw=1, alpha=0.1)
...
def run_mcmc_batch(fns, t0_lim=None, plot=False):
    ...
    def jax_model(t = None, obsflux = None, uncertainties = None, max_flux = None, inc_band_ix = None):
        ...
        sigma_tot = sigma_tot.at[:,inc_band_ix].set(jnp.sqrt(uncertainties[:,inc_band_ix]**2 + extra_sigma_g[:,np.newaxis]**2 * extra_sigma[:,np.newaxis]**2))
    ...
    #with numpyro.validation_enabled():
    res = mcmc.run(rng_key, obsflux = fdata_stacked, t=tdata_stacked, uncertainties=ferrdata_stacked, max_flux=max_flux, inc_band_ix=inc_band_ix)
    ...
    if plot:
        for i in range(len(tdata_stacked)):
            ...
            ignore_idx = (ferrdata_stacked[i] == 1e10)
            ...
            model_i = np.array([{k: posterior_samples[k][j,i] for k in posterior_samples.keys()} for j in range(len(posterior_samples['log_tau_fall']))])
            ...
            for sample in model_i[:30]:
                plt.plot(trange_fine, flux_from_posteriors(trange_fine, sample, max_flux[i])[0], c="g", lw=1, alpha=0.1)
                plt.plot(trange_fine, flux_from_posteriors(trange_fine, sample, max_flux[i])[1], c="r", lw=1, alpha=0.1)
...
def flux_from_posteriors(t, params, max_flux):
    ...
    t0, log_tau_rise, log_tau_fall, log_extra_sigma = params['t0'], params['log_tau_rise'], params['log_tau_fall'], params['log_extra_sigma']
    ...
    t0_g, tau_rise_g, tau_fall_g, extra_sigma_g = params['t0_g'], params['tau_rise_g'], params['tau_fall_g'], params['extra_sigma_g']
    ...
    flux_r = flux_const * ( (1-sigmoid) * (1 - beta*phase) + sigmoid * (1 - beta*gamma) * jnp.exp(-(phase-gamma)/tau_fall) )
    ...
    flux_g = flux_const_b * ( (1-sigmoid_b) * (1 - beta_b*phase_b) + sigmoid_b * (1 - beta_b*gamma_b) * jnp.exp(-(phase_b-gamma_b)/tau_fall_b) )
...
def main_loop_single_file(test_fn, output_dir=FITS_DIR):
    ...
    eq_samples = run_mcmc(test_fn, sampler="svi", plot=True)
...
if __name__ == "__main__":
    all_fns = glob.glob(os.path.join(DATA_DIRS[0],"ZTF22*.npz"))
    Parallel(n_jobs=-1)(delayed(main_loop_single_file)(x) for x in all_fns)
    ...
    while True:
        print(ct)
        end_idx = min(ct+BATCH_NUM, len(all_fns))
        main_loop_directory(all_fns[ct:end_idx])
        break
        if end_idx < ct + BATCH_NUM:
            break
        ct += BATCH_NUM
    """
